...
function renderCalendar() {
  ...
  const heading = document.querySelector("#calendario h2");
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const labels = items.map(item => `<span class="day-charge ${weight}" title="${escapeHtml(item.name)} — ${brl.format(Number(item.amount))}">${escapeHtml(item.name)} · ${brl.format(Number(item.amount))}</span>`).join("");
  ...
  if (onboardingState.step === 0) {
    ...
    content.innerHTML = `
      <div class="onboarding-body">
        ...
        <p>A Loopa vai priorizar próximas cobranças, testes grátis, aumentos e possíveis desperdícios — não apenas uma lista de serviços.</p>
        <div class="choice-grid">
          <div class="choice selected"><strong>Próximos 7 dias</strong><br><span style="color:var(--muted)">cobranças e urgências em primeiro plano</span></div>
          <div class="choice selected"><strong>Visão mensal + anual</strong><br><span style="color:var(--muted)">para o custo pequeno não ficar invisível</span></div>
        </div>
        <div class="onboarding-actions">
          <button class="text-button" data-back>Voltar</button>
...
function bindOnboardingControls() {
  ...
  choiceGroup?.querySelectorAll(".choice").forEach(button => {
    button.addEventListener("click", () => {
      ...
      const field = choiceGroup.dataset.choice;
      onboardingState[field] = button.dataset.value;
...
function setupEvents() {
  document.querySelector("#addButton").addEventListener("click", () => openModal("addModal"));
  ...
  document.querySelector("#sortButton").addEventListener("click", event => {
    ...
    event.currentTarget.textContent = ascendingSort ? "Ordenar por próxima cobrança ↓" : "Ordenar por próxima cobrança ↑";
    renderSubscriptions();
  });
  ...
  document.querySelector("#statementFile").addEventListener("change", async event => {
    ...
    try {
      ...
      importCandidates = scanCsv(text);
      showImportCandidates(importCandidates);
    } catch {
      showImportCandidates([]);
...
function updateDate() {
  const label = document.querySelector(".topbar .eyebrow");
  if (!label) return;
